...
# Load Metrics
# -------------------------
acc = np.array(torch.load("resnet_accuracy.pt"), dtype=np.float32)
conf = np.array(torch.load("confidence_1c.pt"), dtype=np.float32)
disp, rank = [], []

EPOCHS = len(acc)
for e in range(EPOCHS):
    E = np.load(f"resnet_emb_epoch_{e}.npy")
...
plt.title("Phase-1C: Early Silent Failure Detection")
plt.legend()
plt.tight_layout()
plt.savefig("phase1c_results.png", dpi=300)
plt.show()
